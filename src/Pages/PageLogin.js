      ...
      <div className="flex">
        <form className="flex flex-col space-y-4 items-center">
          ...
          <div className="flex flex-col w-full px-2">
            ...
            <input type="email" name="email" id="email" className="border py-1 rounded-lg" />
          </div>

          <div className="flex flex-col w-full px-2">
            <label className="text-xs">Senha</label>
            <div className="flex justify-between space-x-1.5 items-center">
              <input type={showPassword ? "text" : "password"} name="password" id="password" className="border py-1 w-full rounded-lg" value={password} onChange={(e) => setPassword(e.target.value)} />
              <img src={showPassword ? ocultar : ver} className="w-5 h-5" onClick={handlePasswordToggle} />
            </div>
          </div>

          <div className="flex flex-col items-center w-full px-2 space-y-2">
            <button type="submit" className="bg-orange text-white w-full rounded-sm shadow-lg hover:scale-105 py-1 duration-75">
              Entrar
            </button>
